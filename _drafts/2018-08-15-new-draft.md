...
time_step = 120
x_train, y_train = create_dataset(train, time_step=time_step)
x_test, y_test = create_dataset(test, time_step=time_step)
...
- 그리고 우리가 마든 함수 **create_dataset**으로 x과 y를 time_step단위로 나눈다.
